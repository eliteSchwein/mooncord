...
var executeCommand = (function(command,channel,user,guild,discordClient,websocketConnection){
    ...
    for(var temp in temps){
        if(temp.includes("temperature_sensor")){
            ...
            debug=debug.concat("**♨"+temp+":**\n`Current:"+temps[temp].temperatures[temps[temp].temperatures.length-1]+"°C` `Target:"+temps[temp].targets[temps[temp].targets.length-1]+"°C` `Power:"+temps[temp].powers[temps[temp].powers.length-1]+"%`\n\n")
        }else if(temp.includes("temperature_fan")){
            debug=debug.concat("**❄"+temp+"**:\n`Current:"+temps[temp].temperatures[temps[temp].temperatures.length-1]+"°C` `Target:"+temps[temp].targets[temps[temp].targets.length-1]+"°C` `Speed:"+temps[temp].speeds[temps[temp].speeds.length-1]+"`\n\n")
        }
    }
    console.log(temps)
    channel.send(debug)
