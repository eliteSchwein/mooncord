...
var executeCommand = (function(command,channel,user,guild,discordClient,websocketConnection){
    ...
    for(var temp in temps){
        if(temp.includes("temperature_sensor")){
            ...
            debug=debug.concat("**♨"+temp+":**\n`Current:"+temps[temp].temperatures[0]+"` `Target:"+temps[temp].targets[0]+"°C` `"+temps[temp].powers[0]+"%`\n\n")
        }else if(temp.includes("temperature_fan")){
            debug=debug.concat("**❄"+temp+"**:\n`Current:"+temps[temp].temperatures[0]+"` `Target:"+temps[temp].targets[0]+"°C` `"+temps[temp].speeds[0]+"`\n\n")
        }
    }
    console.log(temps)
    channel.send(debug)
