...
var getModule = (async function(discordClient,channel,guild){
    ...
    discordClient.user.setActivity("Pausing...",{type: "WATCHING"})
...
function sendMessage(channel,theme){
    readTemplateFile('./themes/'+theme+'/templates/print_pause.html',async function (err,templatefile){
        if(err){
            channel.send("The File `templates/print_pause.html` \ncouldn't be found in the Theme:\n`"+theme+"`")
            return
        }
